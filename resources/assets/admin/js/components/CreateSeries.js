...
export default {
    ...
    methods: {
        ...
        store() {
            this.isloading = 1,
                axios.post(route('admin.series.store'), this.form)
                    .then((res) => {
                        if (res.data.success) {
        ...
        show() {
            axios.post(route('admin.series.show'))
                .then((res) => {
        ...
        update() {
            this.isloading = 1,
                axios.post(route('admin.series.update'), this.form)
                    .then((res) => {
                        if (res.data.success) {
                            this.$toast.success(res.data.message, {
                                position: "bottom-left"
        ...
        destroy(id, name) {
            this.$swal({
                title: 'Are you sure?',
                html: `Really! Do yo want to remove <b>${name}</b> series?`,
                ...
                if (result.isConfirmed) {
                    ...
                    axios.delete(route("admin.series.destroy", { id: id }))
                        .then((res) => {
                            this.show();
                            this.$swal(
                                'Deleted!',
        ...
        openPager(slug) {
            let url = route('series', { series: slug });
            /** Open New Tab */
