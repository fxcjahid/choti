<div class="row">
    <div class="col-md-8">
        ...
        <div
            class="link-field category-field {{ old('type', $menuItem->type ?? 'category') !== 'category' ? 'hide' : '' }}">
            {{ Form::select('category_id', trans('menu::attributes.category_id'), $errors, $categories, $menuItem, ['required' => true]) }}
        </div>

        <div class="link-field blog-field {{ old('type', $menuItem->type) !== 'blog' ? 'hide' : '' }}">
            {{ Form::select('blog_id', trans('menu::attributes.blog_id'), $errors, $blogs, $menuItem, ['required' => true]) }}
        </div>
